...
and
    ...
    * Events
    *****************************************************************)
    [<Inline "jQuery($this.element.elt).datepicker('option',{beforeShow: function (x,y) {return ($f(x))(y);}})">]
    member private this.onBeforeShow(f : Dom.Element -> Datepicker -> DatepickerConfiguration) = ()

    [<Inline "jQuery($this.element.elt).datepicker('option',{beforeShowDay: function (x,y) {return ($f(x));}})">]
    member private this.onBeforeShowDay (f: Date -> array<obj>)  = () 
    ...
    [<JavaScript>]
    member this.OnBeforeShow(f: Datepicker -> DatepickerConfiguration) : unit =
    ...
    [<JavaScript>]
    member this.OnBeforeShowDay(f: Date -> array<obj>) : unit =
        this.OnAfterRender(fun _ -> this.onBeforeShowDay f)
        |> ignore

    [<JavaScript>]
    member this.OnChangeMonthYear(f: int -> int -> Datepicker -> unit) : unit =
        this.OnAfterRender(fun _ -> this.onChangeMonthYear f)
        |> ignore
