...
type Accordion[<JavaScript>] internal () =
    ...
    *****************************************************************)
    ...
    [<JavaScript>]
    [<Name "New1">]
    static member New (els : List<string * Doc>, conf: AccordionConfiguration): Accordion =
        ...
        let panel =
            let elsDoc =
                ...
                |> List.concat
            div [on.afterRender (fun el -> AccordianInternal.Init(el, conf)) ]
                elsDoc
        acc.element <- (panel :?> Elt)
        acc

    /// Create an accordion with default configuration settings.
    [<JavaScript>]
    [<Name "New2">]
    static member New (els : List<string * Doc>): Accordion =
    ...
    *****************************************************************)
    ...
    [<Inline "jQuery($this.element.elt).accordion('destroy')">]
    member this.Destroy() = ()

    /// Disable the accordion.
    [<Inline "jQuery($this.element.elt).accordion('disable')">]
    member this.Disable () = ()

    /// Enables the accordion.
    [<Inline "jQuery($this.element.elt).accordion('enable')">]
    member this.Enable () = ()

    [<Inline "jQuery($this.element.elt).accordion('widget')">]
    ...
    [<Inline "jQuery($this.element.elt).accordion('option', $name, $value)">]
    member this.Option (name: string, value: obj) = ()

    /// Gets or sets any accordion option.
    [<Inline "jQuery($this.element.elt).accordion('option', $name)">]
    member this.Option (name: string) = X<obj>

    /// Gets all options.
    [<Inline "jQuery($this.element.elt).accordion('option')">]
    member this.Option () = X<AccordionConfiguration>

    /// Sets one or more options.
    [<Inline "jQuery($this.element.elt).accordion('option', $options)">]
    ...
    [<Inline "jQuery($this.element.elt).accordion('option', 'active', $index)">]
    member this.Activate (index: int) = ()

    /// Pass false to close all content parts (only possible with collapsible=true).
    [<Inline "jQuery($this.element.elt).accordion('option', 'active', $keep_open)">]
    ...
    [<Inline "jQuery($this.element.elt).accordion('refresh')">]
    member this.Refresh () = ()

    (****************************************************************
    * Events
    *****************************************************************)
    [<Inline "jQuery($this.element.elt).bind('accordioncreate', function (x,y) {($f(x))(y);})">]
    member private this.onCreate(f : JQuery.Event -> AccordionCreateEvent -> unit) = ()

    [<Inline "jQuery($this.element.elt).bind('accordionbeforeactivate', function (x,y) {($f(x))(y);})">]
    member private this.onBeforeActivate(f : JQuery.Event -> AccordionChangeEvent -> unit) = ()

    [<Inline "jQuery($this.element.elt).bind('accordionactivate', function (x,y) {($f(x))(y);})">]
