...
type Menu[<JavaScript>]internal () =
    ...
    [<JavaScript>]
    [<Name "New1">]
    static member New (el: Elt, conf: MenuConfiguration): Menu =
        let d = new Menu()
        d.element <-
            el.OnAfterRender(fun el ->
                MenuInternal.Init(el, conf)
            )
        d
    ...
    *****************************************************************)
    ...
    [<Inline "jQuery($this.element.elt).menu('destroy')">]
    member this.Destroy() = ()

    /// Disables the menu.
    [<Inline "jQuery($this.element.elt).menu('disable')">]
    member this.Disable () = ()

    /// Enables the menu.
    [<Inline "jQuery($this.element.elt).menu('enable')">]
    member this.Enable () = ()

    /// Sets menu option.
    [<Inline "jQuery($this.element.elt).menu('option', $name, $value)">]
    member this.Option (name: string, value: obj) = ()

    /// Gets menu option.
    [<Inline "jQuery($this.element.elt).menu('option', $name)">]
    member this.Option (name: string) = X<obj>

    /// Gets all options.
    [<Inline "jQuery($this.element.elt).menu('option')">]
    member this.Option () = X<MenuConfiguration>

    /// Sets one or more options.
    [<Inline "jQuery($this.element.elt).menu('option', $options)">]
    member this.Option (options: MenuConfiguration) = X<unit>

    [<Inline "jQuery($this.element.elt).menu('widget')">]
    ...
    [<Inline "jQuery($this.element.elt).menu('blur')">]
    member this.Blur () = ()

    /// Removes focus from the menu.
    [<Inline "jQuery($this.element.elt).menu('blur', $e)">]
    member this.Blur (e: JQuery.Event) = ()

    /// Closes the currently active sub-menu.
    [<Inline "jQuery($this.element.elt).menu('collapse')">]
    member this.Collapse () = ()

    /// Closes the currently active sub-menu.
    [<Inline "jQuery($this.element.elt).menu('collapse', $e)">]
    member this.Collapse (e: JQuery.Event) = ()

    /// Closes all open sub-menus.
    [<Inline "jQuery($this.element.elt).menu('collapseAll')">]
    member this.CollapseAll () = ()

    /// Closes all open sub-menus.
    [<Inline "jQuery($this.element.elt).menu('collapseAll', $e)">]
    member this.CollapseAll (e: JQuery.Event) = ()

    /// Closes all open sub-menus.
    [<Inline "jQuery($this.element.elt).menu('collapseAll', $e, $all)">]
    member this.CollapseAll (e: JQuery.Event, all: bool) = ()

    /// Opens the sub-menu below the currently active item, if one exists.
    [<Inline "jQuery($this.element.elt).menu('expand')">]
    member this.Expand () = ()

    /// Opens the sub-menu below the currently active item, if one exists.
    [<Inline "jQuery($this.element.elt).menu('expand', $e)">]
    ...
    [<Inline "jQuery($this.element.elt).menu('focus', $item)">]
    member this.Focus (item: JQuery.JQuery) = ()

    /// Activates a particular menu item, begins opening any sub-menu if present and triggers the menu's focus event.
    [<Inline "jQuery($this.element.elt).menu('focus', $e, $item)">]
    ...
    [<Inline "jQuery($this.element.elt).menu('isFirstItem')">]
    ...
    [<Inline "jQuery($this.element.elt).menu('isLastItem')">]
    member this.IsLastItem () = false

    /// Moves active state to next menu item.
    [<Inline "jQuery($this.element.elt).menu('next')">]
    member this.Next () = ()

    /// Moves active state to next menu item.
    [<Inline "jQuery($this.element.elt).menu('next', $e)">]
    ...
    [<Inline "jQuery($this.element.elt).menu('nextPage')">]
    member this.NextPage () = ()

    /// Moves active state to first menu item below the bottom of a scrollable menu or the last item if not scrollable.
    [<Inline "jQuery($this.element.elt).menu('nextPage', $e)">]
    member this.NextPage (e: JQuery.Event) = ()

    /// Moves active state to previous menu item.
    [<Inline "jQuery($this.element.elt).menu('previous')">]
    member this.Previous () = ()

    /// Moves active state to previous menu item.
    [<Inline "jQuery($this.element.elt).menu('previous', $e)">]
    member this.Previous (e: JQuery.Event) = ()

    /// Moves active state to first menu item below the bottom of a scrollable menu or the last item if not scrollable.
    [<Inline "jQuery($this.element.elt).menu('previousPage')">]
    member this.PreviousPage () = ()

    /// Moves active state to first menu item below the bottom of a scrollable menu or the last item if not scrollable.
    [<Inline "jQuery($this.element.elt).menu('previousPage', $e)">]
    ...
    [<Inline "jQuery($this.element.elt).menu('refresh')">]
    member this.Refresh () = ()

    /// Selects the currently active menu item, collapses all sub-menus and triggers the menu's select event.
    [<Inline "jQuery($this.element.elt).menu('selectPage')">]
    member this.SelectPage () = ()

    /// Selects the currently active menu item, collapses all sub-menus and triggers the menu's select event.
    [<Inline "jQuery($this.element.elt).menu('selectPage', $e)">]
    member this.SelectPage (e: JQuery.Event) = ()

    (****************************************************************
    * Events
    *****************************************************************)
    [<Inline "jQuery($this.element.elt).bind('menucreate', function (x,y) {$f(x);})">]
    member private this.onCreate(f : JQuery.Event -> unit) = ()

    [<Inline "jQuery($this.element.elt).bind('menufocus', function (x,y) {($f(x))(y.item);})">]
    member private this.onFocus(f : JQuery.Event -> JQuery.JQuery -> unit) = ()

    [<Inline "jQuery($this.element.elt).bind('menublur', function (x,y) {($f(x))(y.item);})">]
    member private this.onBlur(f : JQuery.Event -> JQuery.JQuery -> unit) = ()

    [<Inline "jQuery($this.element.elt).bind('menuselect', function (x,y) {($f(x))(y.item);})">]
    ...
    member this.OnBlur (f : JQuery.Event -> JQuery.JQuery -> unit) =
        this.OnAfterRender (fun _ -> this.onBlur f) |> ignore

    /// Triggered when a menu item is selected.
    [<JavaScript>]
    member this.OnSelect (f : JQuery.Event -> JQuery.JQuery -> unit) =
        this.OnAfterRender (fun _ -> this.onSelect f) |> ignore
