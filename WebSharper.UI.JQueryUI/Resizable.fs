...
type Resizable[<JavaScript>] internal () =
    ...
    *****************************************************************)
    ...
    [<JavaScript>]
    [<Name "New1">]
    static member New (el : Doc, conf: ResizableConfiguration): Resizable =
        let a = new Resizable()
        a.element <-
            (el :?> Elt).OnAfterRender (fun el  ->
                ResizableInternal.New(el, conf)
            )
        a
    ...
    [<JavaScript>]
    [<Name "New2">]
    static member New (el : Doc) : Resizable =
    ...
    *****************************************************************)
    ...
    [<Inline "jQuery($this.element.elt).resizable('destroy')">]
    member this.Destroy() = ()

    /// Disables resizable functionality.
    [<Inline "jQuery($this.element.elt).resizable('disable')">]
    member this.Disable() = ()

    /// Enables resizable functionality.
    [<Inline "jQuery($this.element.elt).resizable('enable')">]
    member this.Enable() = ()

    /// Gets a resizable option.
    [<Inline "jQuery($this.element.elt).resizable('option', $optionName)">]
    member this.Option(optionName: string) = X<obj>

    /// Sets a resizable option.
    [<Inline "jQuery($this.element.elt).resizable('option', $optionName, $value)">]
    member this.Option(optionName: string, value: obj) : unit = ()

    /// Gets all options.
    [<Inline "jQuery($this.element.elt).resizable('option')">]
    member this.Option () = X<ResizableConfiguration>

    /// Sets one or more options.
    [<Inline "jQuery($this.element.elt).resizable('option', $options)">]
    member this.Option (options: ResizableConfiguration) = X<unit>

    [<Inline "jQuery($this.element.elt).resizable('widget')">]
    ...
    * Events
    *****************************************************************)
    [<Inline "jQuery($this.element.elt).bind('resizecreate', function (x,y) {($f(x))(y);})">]
    member private this.onCreate(f : JQuery.Event -> ResizableEvent -> unit) = ()

    [<Inline "jQuery($this.element.elt).bind('resizestart', function (x,y) {($f(x))(y);})">]
    member private this.onStart(f : JQuery.Event -> ResizableEvent -> unit) = ()

    [<Inline "jQuery($this.element.elt).bind('resize', function (x,y) {($f(x))(y);})">]
    member private this.onResize(f : JQuery.Event -> ResizableEvent -> unit) = ()

    [<Inline "jQuery($this.element.elt).bind('resizestop', function (x,y) {($f(x))(y);})">]
